...
{\color{red} - Thanks for this comment. Going through the code again, we realized that we only reported on the non-zero observations. We corrected the figures to include the zero observations, and the text has been updated accordingly. We also adopted the suggestion to report on the RMSE to give an idea of the precision the model is delivering prediction with. However, we do not understand the point of the reviewer around the size of the credible interval. Their step-like appearance is due to the nature of the observation model, being Poisson distributed, so the quantiles have integer values. Furthermore, we are discussing the model performance in terms of the ability to predict the spread and survival of mosquitoes released in the village, and the MRR $5$ allows us to do that.}
